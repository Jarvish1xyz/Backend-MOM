  ...
  <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
    ...
    <div className="mb-8 flex justify-between items-end">
      <div>
        ...
        <p className="text-slate-500 font-medium text-sm mt-1">Manage your public profile and personal details</p>
      </div>

      {!edit ? (
        <button onClick={() => setEdit(true)} className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-600 hover:text-white transition-all shadow-sm">
          Edit Profile
        </button>
      ) : (
        <div className="flex gap-2">
          <button onClick={saveProfile} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">
            Save Changes
          </button>
          <button onClick={() => { setEdit(false); setSelectedFile(null); }} className="bg-white border border-slate-200 text-slate-400 px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all">
            Cancel
          </button>
        </div>
      )}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1">
        <div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm text-center">
          ...
          >
            <img
              src={form.profileImg || "https://ui-avatars.com/api/?name=" + user.username}
              alt="profile"
          ...
          <h3 className="mt-6 font-bold text-xl text-slate-800 uppercase tracking-tight">{user.username}</h3>
