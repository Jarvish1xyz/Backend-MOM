  ...
  return (
    <div className="h-full flex flex-col py-6 bg-white transition-all duration-300">
      ...
      <div className="px-3 space-y-2 flex-1">
        ...
        >
          <svg className={`w-5 h-5 shrink-0 transition-colors ${pathname === "/" ? "text-white" : "text-slate-400 group-hover:text-blue-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          {!isCollapsed && <span className="text-sm font-bold tracking-tight">Dashboard</span>}
        </button>
        ...
        >
          <svg className={`w-5 h-5 shrink-0 transition-colors ${pathname === "/starred" ? "text-white" : "text-slate-400 group-hover:text-blue-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
          </svg>
          {!isCollapsed && <span className="text-sm font-bold tracking-tight">Starred</span>}
        </button>
        ...
        >
          <svg className={`w-5 h-5 shrink-0 transition-colors ${pathname === "/profile" ? "text-white" : "text-slate-400 group-hover:text-blue-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
